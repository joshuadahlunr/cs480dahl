...
bool OrbitPath::Initialize(const Arguments& args, const std::string& texturePath, glm::vec2 orbitDistance, glm::vec3 orbitalTiltNormal) {
	...
	for(int i = 0; i <= resolution; i++){
		...
		glm::vec3 outerPosition = glm::vec3(cos(angleStep * i) * (orbitDistance.x + width), 0, sin(angleStep * i) * (orbitDistance.y + width));
		glm::vec3 innerPosition = glm::vec3(cos(angleStep * i) * (orbitDistance.x - width), 0, sin(angleStep * i) * (orbitDistance.y - width));
