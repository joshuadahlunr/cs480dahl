...
public:
	...
	virtual void setDirection(glm::vec3 direction) {lightDirection = glm::normalize(direction);}
	...
	void setAttenuationStartDistance(float distance) { lightAttenuationStartDistance = distance; }
...
protected:
	...
	std::string uniformLocationLightAttenuationStartDistance;
...
public:
	...
	float lightAttenuationStartDistance = 20;
...
class SpotLight : public Light {
	// glm::vec3 relativeDirection = {0, -1, 0};
public:
	SpotLight(std::string lightVariable = "lights");

	// void update(float dt) override { lightDirection = glm::transpose(glm::inverse(getParent()->getChildBaseModel())) * glm::vec4(relativeDirection, 0); }

	// void setDirection(glm::vec3 direction) override { relativeDirection = direction; }
